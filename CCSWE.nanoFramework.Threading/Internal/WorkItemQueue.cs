...
namespace CCSWE.nanoFramework.Threading.Internal
{
    ...
    internal struct WorkItemQueue
    {
        ...
        public WorkItemQueue(int depth)
        {
            _depth = depth;
            _get = 0;
            _put = 0;
        ...
        public bool Dequeue(ref WorkItem item)
        {
            if (_size == 0)
            {
                return false;
        ...
        public bool Peek(ref WorkItem item)
        {
            if (_size == 0)
            {
                return false;
        ...
        public bool Peek(ref WorkItem item, int index)
        {
            if (_size <= index)
            {
                return false;
